...
let thcCreator = (~header, ~accessor, ~width) =>
  TypeInfo.theaderCell(~header, ~accessor, ~cell=cellValueRaw, ~width);

let thcCreatorBool = (~header, ~accessor, ~width) =>
  TypeInfo.theaderCell(~header, ~accessor, ~cell=cellValueToString, ~width) /* http://2ality.com/2018/01/lists-arrays-reasonml.html */;
